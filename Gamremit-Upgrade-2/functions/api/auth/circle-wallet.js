  ...
  try {
    ...
    const res = await fetch('https://api.circle.com/v1/w3s/developer/wallets', {
      ...
      body: JSON.stringify({
        ...
        // Note: use MATIC-MUMBAI or check Arc network ID]
      })
    });
    const data = await res.json();
    const wallet = data?.data?.wallets?.[0];
    if (!wallet) return {};
    // Auto-fund with testnet USDC
    try {
      await fetch('https://api.circle.com/v1/faucet/drips', {
        method: 'POST',
        headers: { Authorization: `Bearer ${apiKey}`, 'Content-Type': 'application/json' },
        body: JSON.stringify({ address: wallet.address, blockchain: 'ARC-TESTNET', usdc: true, native: true })
      });
    } catch {}
