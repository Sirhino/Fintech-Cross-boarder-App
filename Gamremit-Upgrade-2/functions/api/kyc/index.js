...
import { getUser, saveUser, getKyc, saveKyc, getAllKyc,
         addNotif, pushAdminNotif, sendTelegram, sendEmailKYC,
...
function kycEmailHtml({ firstName, status, reason }) {
  ...
  return `<!DOCTYPE html><html><body style="font-family:Arial,sans-serif;background:#07090F;color:#fff;padding:32px 24px;max-width:520px;margin:0 auto">
    ...
    ${status==='approved'?`<a href="https://gamremit.xyz/app.html" style="display:inline-block;padding:12px 24px;background:linear-gradient(135deg,#1246F5,#2A5AFF);color:#fff;border-radius:10px;text-decoration:none;font-weight:700;margin-top:8px">Go to Dashboard →</a>`:''}
...
export async function onRequest({ request, env }) {
  ...
  if (request.method === 'POST') {
    ...
    try {
      ...
      await sendEmailKYC({ to: user.email, subject: 'GamRemit — KYC Documents Received',
        html: kycEmailHtml({ firstName: user.firstName, status: 'pending' }) }, env);

      // Admin email restored here — now on Mailjet, a separate quota from
      // Brevo/OTP, so there's no reason to hold this back to bell+Telegram
      // only anymore. Bell + Telegram still fire above for instant visibility.
      await sendEmailKYC({
        to: 'gamremit.arc@gmail.com',
        subject: `🪪 New KYC Submission — ${user.firstName} ${user.lastName}`,
        html: `
          <div style="font-family:sans-serif;max-width:480px;margin:auto">
            <h2 style="color:#1246F5">New KYC Submission</h2>
            <table style="width:100%;border-collapse:collapse;font-size:14px">
              <tr><td style="padding:6px 0;color:#666">Name</td><td style="padding:6px 0;font-weight:600">${user.firstName} ${user.lastName}</td></tr>
              <tr><td style="padding:6px 0;color:#666">Email</td><td style="padding:6px 0;font-weight:600">${user.email}</td></tr>
              <tr><td style="padding:6px 0;color:#666">Document</td><td style="padding:6px 0;font-weight:600">${labels[docType]||docType}</td></tr>
              <tr><td style="padding:6px 0;color:#666">Requested Tier</td><td style="padding:6px 0;font-weight:600">${requestedTierInfo.icon} ${requestedTierInfo.label}</td></tr>
              <tr><td style="padding:6px 0;color:#666">Intended Amount</td><td style="padding:6px 0;font-weight:600">${intendedAmount ? intendedAmount.toLocaleString() + ' ' + intendedCurrency : 'Not specified'}</td></tr>
              ${resubmitCount>0 ? `<tr><td style="padding:6px 0;color:#666">Resubmission</td><td style="padding:6px 0;font-weight:600">#${resubmitCount}</td></tr>` : ''}
              <tr><td style="padding:6px 0;color:#666">Submitted</td><td style="padding:6px 0;font-weight:600">${new Date().toLocaleString('en-GB', { timeZone: 'Africa/Banjul' })}</td></tr>
            </table>
            <p style="margin-top:20px"><a href="https://gamremitagent.pages.dev/admin.html#kyc" style="color:#1246F5">Review in Admin Panel →</a></p>
          </div>`
      }, env);
    } catch(e) { console.error('[kyc:notify]', e.message); }
  ...
  if (request.method === 'PATCH' && isUserId) {
    ...
    if (user) {
      ...
      await sendEmailKYC({ to: user.email,
