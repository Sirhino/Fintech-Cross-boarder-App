...
export async function onRequest({ request, env }) {
  ...
  if (request.method === 'GET' && name && !request.headers.get('Authorization')) {
    ...
    const existing = await kvGet(`arcname:${name}`, env);
  ...
  if (request.method === 'POST') {
    ...
    const existing = await kvGet(`arcname:${rawName}`, env);
    ...
    const walletAddress = user.circleWalletId || body.walletAddress || '0x0000000000000000000000000000000000000000';
    ...
    await kvSet(`arcname:${rawName}`, record, env);
  ...
  if (request.method === 'DELETE' && name) {
    ...
    await kvSet(`arcname:${name}`, null, env);
