...
import { getTx, getAllTxs, saveTx, getUser, saveUser, getRates,
         generateRef, getTier, addNotif, pushAdminNotif, sendTelegram,
...
export async function onRequest({ request, env, params }) {
  ...
  if (request.method === 'POST') {
    ...
    // Get amount in GMD for tier calculation
    let amountInGMD = amount;
    if (from === 'NGN')  amountInGMD = amount * 0.0806; // NGN → GMD
    if (from === 'USD')  amountInGMD = amount * 61.50;  // USD → GMD
    if (from === 'USDC') amountInGMD = amount * 61.73;  // USDC → GMD
    ...
    await pushAdminNotif({
      type: 'tx', title: '💸 New Transfer Submitted',
      body: `${tx.userName} sent ${amount.toLocaleString()} ${from} → ${to}. Ref: ${txRef}`,
      link: '/admin.html#transactions'
    }, env);

    await sendTelegram(
      `💸 *New GamRemit Transfer*\n\n` +
