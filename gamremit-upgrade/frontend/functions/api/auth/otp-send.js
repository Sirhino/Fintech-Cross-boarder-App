...
// Helper: set with TTL (600s = 10 min)
async function kvSetWithTTL(key, value, ttlSeconds, env) {
  if (!env?.UPSTASH_REDIS_REST_URL) return; // dev fallback
  try {
    await fetch(`${env.UPSTASH_REDIS_REST_URL}/setex/${encodeURIComponent(key)}/${ttlSeconds}`, {
      method: 'POST',
      headers: {
